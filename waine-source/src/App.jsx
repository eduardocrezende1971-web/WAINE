...
const C = {
  ...
  goldD:   "#9A7A4A",
  wine:    "#5B0F1A",
  wineL:   "rgba(91,15,26,0.3)",
  sepia:   "#7A5C3A",
  sepiaL:  "rgba(122,92,58,0.15)",
  green:   "#2A4A36",
  greenL:  "rgba(42,74,54,0.2)",
  white:   "#F2EDE2",
};
...
const VINHO0 = [];
const sAccent = s => s==="Branco"?C.gold:s==="Rose"?"#B05070":s==="Espumante"?"#4A8068":C.wine;
const sLabel  = s => s==="Branco"?C.goldL:s==="Rose"?"rgba(176,80,112,0.15)":s==="Espumante"?"rgba(74,128,104,0.15)":C.wineL;
...
const Lightbox = ({ url, onClose }) => {
  if (!url) return null;
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.96)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <button onClick={onClose} style={{position:"absolute",top:20,right:20,background:"none",border:"none",color:C.gold,fontSize:32,cursor:"pointer",lineHeight:1}}>×</button>
...
{
  "historia": "História e filosofia do produtor em 3-4 frases sofisticadas em português.",
  "regiao": "Descrição rica do terroir: clima, topografia, ventos, proximidade de mar/lago/montanhas, solo, altitude. 4-5 frases em português, tom apaixonado e técnico.",
  ...
  "notas": {
    ...
    "estrutura": "Taninos, acidez, corpo em 1 linha",
...
const PAIS_COORDS = {
  "África do Sul": { x:530, y:375 }, "Argentina": { x:265, y:385 },
  "Austrália": { x:820, y:385 }, "Áustria": { x:515, y:170 },
  "Brasil": { x:295, y:335 }, "Chile": { x:250, y:390 },
  "Espanha": { x:458, y:192 }, "Estados Unidos": { x:170, y:195 },
  "França": { x:473, y:182 }, "Grécia": { x:535, y:198 },
  "Hungria": { x:522, y:172 }, "Itália": { x:508, y:192 },
  "Nova Zelândia": { x:878, y:425 }, "Portugal": { x:448, y:198 },
  "Alemanha": { x:488, y:168 }, "Uruguai": { x:282, y:388 },
};
...
const TabMapa = ({ wines }) => {
  ...
  const getR = (g) => 16 + ((g/maxGarrafas) * 20);
  ...
  return (
    <div style={{flex:1,overflowY:"auto",paddingBottom:90}}>
      <div style={{padding:"32px 24px 20px"}}>
        <div style={{fontFamily:"'DM Sans'",fontSize:9,color:C.muted,letterSpacing:"0.2em",textTransform:"uppercase",marginBottom:8}}>MAPA DA ADEGA</div>
        <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:32,fontWeight:300,color:C.text,lineHeight:1}}>
          {Object.keys(porPais).length} <span style={{color:C.muted}}>países</span>
        </div>
      ...
      <div style={{padding:"0 16px",marginBottom:24}}>
        <div style={{background:C.card,borderRadius:12,border:`1px solid ${C.border}`,overflow:"hidden"}}>
          <svg viewBox="0 0 1000 500" style={{width:"100%",display:"block"}}>
            <rect width="1000" height="500" fill="#111113"/>
            <path d="M 80,80 L 260,80 L 280,130 L 260,180 L 220,200 L 200,240 L 180,260 L 150,250 L 120,220 L 90,180 L 70,140 Z" fill="#1E1E22" stroke="#2A2A2E" strokeWidth="1"/>
            <path d="M 180,260 L 220,260 L 230,300 L 210,310 L 190,290 Z" fill="#1E1E22" stroke="#2A2A2E" strokeWidth="1"/>
            <path d="M 220,300 L 320,300 L 340,320 L 330,380 L 310,420 L 280,440 L 250,430 L 230,400 L 220,360 L 210,330 Z" fill="#1E1E22" stroke="#2A2A2E" strokeWidth="1"/>
            <path d="M 430,100 L 560,100 L 570,160 L 550,200 L 510,210 L 470,205 L 440,190 L 420,160 L 425,130 Z" fill="#1E1E22" stroke="#2A2A2E" strokeWidth="1"/>
            <path d="M 460,80 L 530,70 L 540,100 L 490,110 L 460,100 Z" fill="#1E1E22" stroke="#2A2A2E" strokeWidth="1"/>
            <path d="M 450,220 L 580,220 L 590,280 L 570,360 L 540,410 L 510,420 L 490,410 L 470,370 L 450,310 L 440,260 Z" fill="#1E1E22" stroke="#2A2A2E" strokeWidth="1"/>
            <path d="M 560,80 L 850,70 L 870,120 L 840,160 L 780,170 L 720,160 L 660,150 L 600,140 L 570,120 Z" fill="#1E1E22" stroke="#2A2A2E" strokeWidth="1"/>
            <path d="M 560,160 L 640,160 L 650,210 L 620,230 L 580,220 L 555,200 Z" fill="#1E1E22" stroke="#2A2A2E" strokeWidth="1"/>
            <path d="M 660,160 L 720,160 L 730,230 L 700,270 L 670,250 L 650,210 Z" fill="#1E1E22" stroke="#2A2A2E" strokeWidth="1"/>
            <path d="M 720,120 L 860,120 L 870,180 L 840,210 L 790,220 L 750,210 L 720,190 L 710,160 Z" fill="#1E1E22" stroke="#2A2A2E" strokeWidth="1"/>
            <path d="M 770,340 L 900,330 L 920,380 L 900,430 L 850,450 L 800,440 L 770,400 L 760,370 Z" fill="#1E1E22" stroke="#2A2A2E" strokeWidth="1"/>
            <path d="M 900,410 L 915,410 L 920,440 L 905,445 L 898,430 Z" fill="#1E1E22" stroke="#2A2A2E" strokeWidth="1"/>
            {paisesComCoords.map(({ pais, garrafas, rotulos, coords }) => {
              const r = getR(garrafas);
              const isSel = tooltip?.pais === pais;
              return (
                <g key={pais} style={{cursor:"pointer"}} onClick={()=>setTooltip(isSel?null:{pais,garrafas,rotulos,coords})}>
                  <circle cx={coords.x} cy={coords.y} r={r} fill={isSel?"#C9A46E":"#9A7A4A"} fillOpacity={0.9} stroke="#C9A46E" strokeWidth="1.5"/>
                  <text x={coords.x} y={coords.y} textAnchor="middle" dominantBaseline="central" fill="#0D0D0F" fontSize={r>24?13:10} fontFamily="'Cormorant Garamond',serif" fontWeight="600">{garrafas}</text>
                </g>
              );
            })}
            {tooltip&&(
              <g>
                <rect x={Math.min(tooltip.coords.x+10,820)} y={tooltip.coords.y-42} width="170" height="54" rx="4" fill="#1A1A1D" stroke="#C9A46E" strokeWidth="0.5" fillOpacity="0.95"/>
                <text x={Math.min(tooltip.coords.x+95,905)} y={tooltip.coords.y-22} textAnchor="middle" fill="#C9A46E" fontSize="10" fontFamily="'DM Sans',sans-serif" fontWeight="500" letterSpacing="2">{tooltip.pais.toUpperCase()}</text>
                <text x={Math.min(tooltip.coords.x+95,905)} y={tooltip.coords.y-4} textAnchor="middle" fill="#6B6058" fontSize="10" fontFamily="'DM Sans',sans-serif">{tooltip.rotulos} rótulo{tooltip.rotulos!==1?"s":""} · {tooltip.garrafas} garrafa{tooltip.garrafas!==1?"s":""}</text>
              </g>
            )}
          </svg>
        </div>
      ...
      <div style={{padding:"0 24px"}}>
        ...
        {Object.entries(porPais).sort((a,b)=>b[1].garrafas-a[1].garrafas).map(([pais,dados])=>(
          <div key={pais} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:8,padding:"16px 18px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,color:C.text,marginBottom:2}}>{pais}</div>
            ...
            <div style={{width:38,height:38,borderRadius:"50%",background:C.goldL,border:`1px solid ${C.gold}`,display:"flex",alignItems:"center",justifyContent:"center"}}>
              <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:17,fontWeight:600,color:C.gold}}>{dados.garrafas}</span>
            </div>
          </div>
        ))}
        {Object.keys(porPais).length===0&&(
          <div style={{textAlign:"center",padding:"48px 0",fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontStyle:"italic",color:C.muted}}>Adicione vinhos para ver o mapa</div>
        )}
      </div>
    </div>
  );
  ...
  if (detail) {
    ...
    return (
      <div style={{position:"fixed",inset:0,background:C.bg,zIndex:50,overflowY:"auto"}}>
        ...
        <div style={{position:"sticky",top:0,background:C.bg,borderBottom:`1px solid ${C.border}`,padding:"16px 24px",display:"flex",alignItems:"center",zIndex:10}}>
        ...
        <div style={{padding:"32px 24px 80px"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:8}}>
            ...
            <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:52,fontWeight:300,color:C.gold,lineHeight:1}}>{w.rating}</span>
          </div>
          <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:30,fontWeight:400,color:C.text,marginBottom:4}}>{w.name}</div>
          ...
          {w.location&&<div style={{fontFamily:"'DM Sans'",fontSize:11,color:C.muted,marginBottom:24,letterSpacing:"0.04em"}}>📍 {w.location} · {w.date}</div>}
          <div style={{background:C.card,borderRadius:8,padding:"22px",border:`1px solid ${C.border}`}}>
            <div style={{fontFamily:"'DM Sans'",fontSize:9,color:C.gold,letterSpacing:"0.2em",textTransform:"uppercase",marginBottom:14}}>MEMÓRIA</div>
  ...
  return (
    <div style={{flex:1,overflowY:"auto",paddingBottom:90}}>
      <div style={{padding:"32px 24px 0",marginBottom:24}}>
        <div style={{fontFamily:"'DM Sans'",fontSize:9,color:C.muted,letterSpacing:"0.2em",textTransform:"uppercase",marginBottom:8}}>MEMÓRIAS</div>
        <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:32,fontWeight:300,color:C.text,lineHeight:1}}>
          Top <span style={{color:C.muted}}>experiências</span>
        </div>
      ...
      <div style={{padding:"0 24px"}}>
        {top20.length === 0 ? (
          <div style={{textAlign:"center",padding:"72px 0"}}>
            ...
            <div style={{fontFamily:"'DM Sans'",fontSize:11,color:C.muted,marginTop:12,letterSpacing:"0.04em"}}>Adicione uma memória ao registrar um vinho</div>
          </div>
        ) : top20.map((w, i) => (
          <div key={w.id} onClick={()=>setDetail(w)}
            style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,marginBottom:10,cursor:"pointer",overflow:"hidden",display:"flex",alignItems:"stretch"}}>
            <div style={{width:56,background:i===0?"#9A7A4A":i===1?"#5A4A2A":i===2?C.sepia:C.card2,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:4,flexShrink:0}}>
              <span style={{fontFamily:"'DM Sans'",fontSize:10,color:i<3?"#0D0D0F":C.muted,fontWeight:500,letterSpacing:"0.04em"}}>#{i+1}</span>
              <span style={{fontSize:13}}>❤️</span>
              <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:600,color:i<3?"#0D0D0F":C.gold}}>{w.rating}</span>
            </div>
            <div style={{flex:1,padding:"14px 16px",minWidth:0}}>
              <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:400,color:C.text,lineHeight:1.2,marginBottom:3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{w.name}</div>
            ...
            {w.photos?.[0]&&<img src={w.photos[0]} style={{width:64,objectFit:"cover",flexShrink:0,opacity:0.85}} />}
          </div>
        ))}
      </div>
    </div>
      ...
      <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
        {photos.map((url,i)=>(
          <div key={i} style={{position:"relative"}}>
            <img src={url} style={{width:88,height:118,objectFit:"cover",borderRadius:4,border:`1px solid ${C.border}`,display:"block"}} />
            <button onClick={()=>onChange(photos.filter((_,j)=>j!==i))} style={{position:"absolute",top:-8,right:-8,width:22,height:22,borderRadius:"50%",background:C.wine,border:`2px solid ${C.bg}`,color:C.text,fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>×</button>
          </div>
        ))}
        {photos.length<max&&(
          <div onClick={()=>!busy&&ref.current.click()} style={{width:88,height:118,border:`1px dashed ${busy?C.gold:C.border}`,borderRadius:4,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",cursor:busy?"wait":"pointer",gap:6,background:busy?C.goldL:"transparent"}}>
            <span style={{fontSize:22,opacity:0.5}}>{busy?"...":"📷"}</span>
            <span style={{fontFamily:"'DM Sans'",fontSize:9,color:C.muted,letterSpacing:"0.1em"}}>{busy?"ENVIANDO":"FOTO"}</span>
          </div>
        )}
      </div>
      {err&&<div style={{fontFamily:"'DM Sans'",fontSize:11,color:"#C04040",marginTop:8}}>{err}</div>}
...
const Qty = ({ value, onChange, accent }) => (
  <div style={{display:"flex",alignItems:"center",background:C.card2,borderRadius:6,border:`1px solid ${C.border}`,overflow:"hidden"}}>
    ...
    <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,fontWeight:400,color:accent||C.gold,minWidth:30,textAlign:"center"}}>{value}</span>
...
const VoiceButton = ({ onResult }) => {
  ...
  return (
    <button onClick={toggle}
      style={{width:36,height:36,borderRadius:"50%",border:`1px solid ${ouvindo?C.gold:C.border}`,background:ouvindo?C.goldL:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
...
const WineDetail = ({ w, onBack, onUpdate, onDelete }) => {
  ...
  const accentColor = isDegustado ? C.sepia : C.gold;
...
Retorne APENAS JSON válido sem markdown com os campos que mudaram:
{
  ...
  "notas":{"aromas":"...","paladar":"...","estrutura":"...","guarda":"...","harmonizacao":"..."}
      ...
      const parsed = JSON.parse(d.content[0].text.replace(/```json|```/g,"").trim());
      setF(p=>({...p,...parsed}));
      setAiMsg("✓ Corrigido. Revise e salve.");
  ...
  return (
    <div style={{position:"fixed",inset:0,background:C.bg,zIndex:50,overflowY:"auto"}}>
      ...
      <div style={{position:"sticky",top:0,background:C.bg,borderBottom:`1px solid ${C.border}`,padding:"16px 24px",display:"flex",justifyContent:"space-between",alignItems:"center",zIndex:10}}>
        ...
        <button onClick={()=>editing?save():setEditing(true)} style={{background:editing?C.gold:"none",border:`1px solid ${editing?C.gold:C.border}`,borderRadius:4,padding:"7px 18px",fontFamily:"'DM Sans'",fontSize:10,letterSpacing:"0.1em",color:editing?C.bg:C.muted,cursor:"pointer"}}>
      ...
      <div style={{padding:"32px 24px 100px"}}>
        {isDegustado&&(
          <div style={{background:C.sepiaL,border:`1px solid ${C.sepia}`,borderRadius:6,padding:"10px 16px",marginBottom:24,display:"flex",alignItems:"center",gap:8}}>
            ...
            <div style={{display:"flex",gap:12,marginBottom:32}}>
              {f.photos.map((url,i)=>(
                <img key={i} src={url} onClick={()=>setLightbox(url)}
                  style={{width:128,height:170,objectFit:"cover",borderRadius:6,border:`1px solid ${C.border}`,filter:isDegustado?"sepia(50%) brightness(0.7)":"none",cursor:"zoom-in"}} />
              ))}
            </div>
          )
        }

        <div style={{width:24,height:1,background:C.gold,marginBottom:24,opacity:0.5}} />
        ...
        {editing
          ? <input value={f.name} onChange={e=>set("name",e.target.value)} style={{width:"100%",fontFamily:"'Cormorant Garamond',serif",fontSize:30,fontWeight:400,color:C.text,border:"none",borderBottom:`1px solid ${C.border}`,background:"none",outline:"none",marginBottom:8,boxSizing:"border-box"}} />
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:32}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            {editing
              ...
              : <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:64,color:accentColor,fontWeight:300,lineHeight:1}}>{w.rating}</div>
            }
          </div>
        ...
        {editing&&(
          <div style={{marginBottom:28}}>
            ...
            <input type="range" min={0} max={100} value={f.rating} onChange={e=>set("rating",+e.target.value)} style={{width:"100%",accentColor:C.gold}} />
          </div>
        )}
        ...
        {w.location&&<div style={{fontFamily:"'DM Sans'",fontSize:11,color:C.muted,marginTop:8,marginBottom:32,letterSpacing:"0.04em"}}>📍 {w.location}</div>}

        {(editing||w.historia)&&(
          <div style={{marginBottom:20,padding:"20px",background:C.card,borderRadius:8,border:`1px solid ${C.border}`}}>
            <div style={{fontFamily:"'DM Sans'",fontSize:9,color:C.gold,letterSpacing:"0.16em",textTransform:"uppercase",marginBottom:12}}>O PRODUTOR</div>
            {editing
              ? <textarea value={f.historia||""} onChange={e=>set("historia",e.target.value)} rows={4} style={{width:"100%",fontFamily:"'Cormorant Garamond',serif",fontSize:16,fontStyle:"italic",color:C.text,border:`1px solid ${C.border}`,borderRadius:4,background:C.card2,outline:"none",padding:10,resize:"none",lineHeight:1.8,boxSizing:"border-box"}} />
        ...
        {(editing||w.regiao)&&(
          <div style={{marginBottom:20,padding:"20px",background:C.card,borderRadius:8,border:`1px solid ${C.border}`}}>
            <div style={{fontFamily:"'DM Sans'",fontSize:9,color:"#4A8068",letterSpacing:"0.16em",textTransform:"uppercase",marginBottom:12}}>A REGIÃO</div>
            {editing
              ? <textarea value={f.regiao||""} onChange={e=>set("regiao",e.target.value)} rows={4} placeholder="Terroir, clima, topografia, ventos..." style={{width:"100%",fontFamily:"'Cormorant Garamond',serif",fontSize:16,fontStyle:"italic",color:C.text,border:`1px solid ${C.border}`,borderRadius:4,background:C.card2,outline:"none",padding:10,resize:"none",lineHeight:1.8,boxSizing:"border-box"}} />
            ...
            {editing
              ? <div style={{display:"flex",flexDirection:"column",gap:14}}>
                  {Object.keys(labelsNotas).map(k=>(
                    <div key={k}>
                      <div style={{fontFamily:"'DM Sans'",fontSize:9,color:C.muted,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:5}}>{labelsNotas[k]}</div>
        ...
        {editing&&(
          <div style={{marginBottom:20,padding:"18px",background:C.card,borderRadius:8,border:`1px solid ${C.border}`}}>
            <div style={{fontFamily:"'DM Sans'",fontSize:9,color:C.gold,letterSpacing:"0.16em",textTransform:"uppercase",marginBottom:12}}>✦ CORRIGIR COM IA</div>
            <textarea value={aiPrompt} onChange={e=>setAiPrompt(e.target.value)} rows={2}
              placeholder='ex: "Este vinho é da África do Sul, não da França."'
              style={{width:"100%",fontFamily:"'DM Sans'",fontSize:13,color:C.text,border:`1px solid ${C.border}`,borderRadius:4,background:C.card2,outline:"none",padding:"10px 12px",resize:"none",boxSizing:"border-box",marginBottom:10}} />
            <button onClick={corrigirComIA} disabled={aibusy||!aiPrompt.trim()} style={{width:"100%",padding:"10px",background:C.gold,border:"none",borderRadius:4,color:C.bg,fontFamily:"'DM Sans'",fontSize:11,letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer",opacity:!aiPrompt.trim()?0.4:1}}>
              {aibusy?"Corrigindo...":"Corrigir com IA"}
            </button>
            {aiMsg&&<div style={{fontFamily:"'DM Sans'",fontSize:11,color:aiMsg.startsWith("✓")?"#4A8068":C.gold,marginTop:8}}>{aiMsg}</div>}
          </div>
        )}
      </div>
    </div>
...
const AddWine = ({ onClose, onSave }) => {
  const [f, setF] = useState({ name:"", producer:"", vintage:2024, region:"", country:"África do Sul", grapes:"", style:"Tinto", bottles:1, rating:50, memory:"", location:"", accent:C.gold, special:false, photos:[], historia:"", regiao:"", alcohol:"", notas:null });
  const [busy, setBusy] = useState(false);
          ...
          messages:[{ role:"user", content:`Você é um sommelier especialista. Para o vinho "${f.name}" do produtor "${f.producer}" safra ${f.vintage}, retorne APENAS JSON válido sem markdown:
{
  ...
  "regiao": "Descrição rica do terroir: clima, topografia, ventos, proximidade de mar/montanhas, solo. 4-5 frases em português.",
  "notas": {
    "aromas": "Aromas principais em 1-2 linhas",
    "paladar": "Sensações na boca em 1-2 linhas",
    "estrutura": "Taninos, acidez, corpo em 1 linha",
    ...
    "harmonizacao": "Harmonização gastronômica em 1 linha"
      ...
      const d = await res.json();
      if(d.error) { setStatus({msg:"Erro: "+d.error.message,ok:false}); setBusy(false); return; }
      const parsed = JSON.parse(d.content[0].text.replace(/```json|```/g,"").trim());
      setF(p=>({...p,...parsed}));
  ...
  const inp = (label,k,type="text",ph="") => (
    <div style={{marginBottom:18}}>
      <div style={{fontFamily:"'DM Sans'",fontSize:9,color:C.muted,letterSpacing:"0.16em",textTransform:"uppercase",marginBottom:8}}>{label}</div>
      <input type={type} value={f[k]} placeholder={ph} onChange={e=>set(k,type==="number"?+e.target.value:e.target.value)}
        style={{width:"100%",background:C.card2,border:`1px solid ${C.border}`,borderRadius:4,padding:"12px 14px",color:C.text,fontFamily:"'DM Sans'",fontSize:14,outline:"none",boxSizing:"border-box"}} />
    </div>
  );
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:100,background:"rgba(0,0,0,0.7)",display:"flex",flexDirection:"column",justifyContent:"flex-end"}}>
      <div style={{background:C.card,borderRadius:"16px 16px 0 0",padding:"28px 24px 52px",maxHeight:"92vh",overflowY:"auto"}}>
        ...
        <div style={{fontFamily:"'DM Sans'",fontSize:11,color:C.muted,marginBottom:28,letterSpacing:"0.04em"}}>Foto + nome + produtor — a IA completa a ficha.</div>
        <PhotoUploader photos={f.photos} onChange={v=>set("photos",v)} />
        {inp("Nome do Vinho","name","text","ex: Paul Sauer")}
        {inp("Produtor","producer","text","ex: Kanonkop")}
        {inp("Safra","vintage","number")}
        <button onClick={enrich} disabled={busy||!f.name||!f.producer}
          style={{width:"100%",padding:14,marginBottom:18,background:status.ok?C.greenL:C.goldL,border:`1px solid ${status.ok?"#4A8068":C.gold}`,borderRadius:6,color:status.ok?"#4A8068":C.gold,fontFamily:"'DM Sans'",fontSize:11,letterSpacing:"0.12em",textTransform:"uppercase",cursor:"pointer",opacity:(!f.name||!f.producer)?0.35:1}}>
          {busy?"✦ Pesquisando...":status.ok?"✓ Ficha gerada":"✦ Gerar ficha com IA"}
        </button>
        {status.msg&&<div style={{fontFamily:"'DM Sans'",fontSize:11,color:status.ok?"#4A8068":C.gold,background:status.ok?C.greenL:C.goldL,padding:"10px 14px",borderRadius:4,marginBottom:18,letterSpacing:"0.04em"}}>{status.msg}</div>}

        {f.historia&&(
          <div style={{marginBottom:18,padding:"18px",background:C.card2,borderRadius:6,border:`1px solid ${C.border}`}}>
            <div style={{fontFamily:"'DM Sans'",fontSize:9,color:C.gold,letterSpacing:"0.16em",textTransform:"uppercase",marginBottom:10}}>O PRODUTOR</div>
            <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:16,fontStyle:"italic",color:C.sub,lineHeight:1.8}}>{f.historia}</div>
          </div>
        )}
        {f.regiao&&(
          <div style={{marginBottom:18,padding:"18px",background:C.card2,borderRadius:6,border:`1px solid ${C.border}`}}>
            <div style={{fontFamily:"'DM Sans'",fontSize:9,color:"#4A8068",letterSpacing:"0.16em",textTransform:"uppercase",marginBottom:10}}>A REGIÃO</div>
        ...
        {status.ok&&(
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:1,background:C.border,marginBottom:18,borderRadius:4,overflow:"hidden"}}>
        ...
        <div style={{marginBottom:18}}>
          ...
          <Qty value={f.bottles} onChange={v=>set("bottles",v)} accent={C.gold} />
        </div>
        <div style={{marginBottom:18}}>
        ...
        <div style={{marginBottom:18}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"flex-start"}}>
            <textarea value={f.memory} onChange={e=>set("memory",e.target.value)} rows={3} placeholder="Onde estava, com quem, o que sentiu..."
              style={{flex:1,background:C.card2,border:`1px solid ${C.border}`,borderRadius:4,padding:"12px 14px",color:C.text,fontFamily:"'Cormorant Garamond',serif",fontSize:16,fontStyle:"italic",outline:"none",resize:"none",boxSizing:"border-box",lineHeight:1.7}} />
            <VoiceButton onResult={txt=>set("memory",(f.memory?f.memory+" ":"")+txt)} />
          </div>
        </div>
        <div style={{marginBottom:18}}>
          ...
          <input value={f.location} onChange={e=>set("location",e.target.value)} placeholder="ex: Kanonkop, Stellenbosch"
            style={{width:"100%",background:C.card2,border:`1px solid ${C.border}`,borderRadius:4,padding:"12px 14px",color:C.text,fontFamily:"'DM Sans'",fontSize:14,outline:"none",boxSizing:"border-box"}} />
        </div>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:28,cursor:"pointer"}} onClick={()=>set("special",!f.special)}>
          <div style={{width:20,height:20,border:`1px solid ${f.special?C.gold:C.border}`,borderRadius:3,background:f.special?C.goldL:"none",display:"flex",alignItems:"center",justifyContent:"center"}}>
            {f.special&&<span style={{color:C.gold,fontSize:13}}>★</span>}
          </div>
        ...
        <div style={{display:"flex",gap:10}}>
          ...
          <button onClick={()=>{if(!f.name)return;onSave({...f,id:Date.now(),date:new Date().toLocaleDateString("pt-BR",{month:"short",year:"numeric"})});}}
            style={{flex:2,padding:14,background:C.gold,border:"none",borderRadius:6,color:C.bg,fontFamily:"'DM Sans'",fontSize:11,letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer",fontWeight:500}}>
            ADICIONAR À ADEGA
          </button>
        </div>
      </div>
...
const TabAdega = ({ wines, setWines }) => {
  ...
  const enriquecerAdega = async () => {
    ...
    for (let i = 0; i < wines.length; i++) {
      ...
      try {
        ...
        setWines([...novasWines]);
      } catch(e) { console.error("Erro:", w.name, e); }
  ...
  return (
    <div style={{flex:1,overflowY:"auto",paddingBottom:90}}>
      ...
      {enriquecendo&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",zIndex:150,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <div style={{background:C.card,borderRadius:12,padding:"36px 32px",width:300,textAlign:"center",border:`1px solid ${C.border}`}}>
            ...
            <div style={{fontFamily:"'DM Sans'",fontSize:11,color:C.muted,marginBottom:20,letterSpacing:"0.04em"}}>{progresso.atual} de {progresso.total} · {progresso.msg}</div>
            <div style={{background:C.card2,borderRadius:2,height:2,overflow:"hidden"}}>
              <div style={{background:C.gold,height:"100%",width:`${(progresso.atual/progresso.total)*100}%`,transition:"width 0.3s",borderRadius:2}} />
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{padding:"32px 24px 0"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
          <div>
            ...
            <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:32,fontWeight:300,color:C.text,lineHeight:1}}>
              {winesAtivos.length} <span style={{color:C.muted}}>rótulos</span>
            </div>
            <div style={{fontFamily:"'DM Sans'",fontSize:11,color:C.muted,marginTop:4}}>{total} garrafas · {winesDegustados.length} degustados</div>
          </div>
          <button onClick={()=>setAdding(true)} style={{background:C.gold,border:"none",borderRadius:"50%",width:46,height:46,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:C.bg,fontSize:22,boxShadow:"0 2px 16px rgba(201,164,110,0.3)"}}>+</button>
        </div>

        {wines.length>0&&!filtroAtivo&&(
          <button onClick={enriquecerAdega} disabled={enriquecendo}
            style={{width:"100%",padding:"12px",marginBottom:24,background:"none",border:`1px solid ${C.border}`,borderRadius:6,color:C.gold,fontFamily:"'DM Sans'",fontSize:10,letterSpacing:"0.14em",textTransform:"uppercase",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
      ...
      {filtroAtivo ? (
        <div>
          <div style={{padding:"0 24px",marginBottom:14}}>
            <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:14}}>
              ...
              <span style={{fontFamily:"'DM Sans'",fontSize:10,color:filtroAtivo==="Degustados"?C.sepia:C.gold,background:filtroAtivo==="Degustados"?C.sepiaL:C.goldL,padding:"6px 14px",borderRadius:20,letterSpacing:"0.08em"}}>
                {filtroAtivo} · {winesFiltrados.length}
              </span>
            </div>
            <div style={{position:"relative"}}>
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar..."
                style={{width:"100%",background:C.card,border:`1px solid ${C.border}`,borderRadius:6,padding:"10px 14px 10px 36px",color:C.text,fontFamily:"'DM Sans'",fontSize:13,outline:"none",boxSizing:"border-box"}} />
          ...
          <div style={{padding:"0 24px"}}>
            {winesFiltrados.length===0
              ...
              : winesFiltrados.map(w=>{
                ...
                return (
                  <div key={w.id} onClick={()=>setDetail(w)} style={{background:C.card,border:`1px solid ${isDeg?C.sepia+"40":C.border}`,borderRadius:8,marginBottom:10,cursor:"pointer",overflow:"hidden"}}>
                    {w.photos?.[0]&&(
                      <div style={{position:"relative",height:160,overflow:"hidden"}}>
                        <img src={w.photos[0]} style={{width:"100%",height:"100%",objectFit:"cover",filter:isDeg?"sepia(60%) brightness(0.6)":"none"}} />
                        ...
                        <div style={{position:"absolute",bottom:14,left:16,right:16,display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
                          <div>
                            <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,fontWeight:400,color:"#fff",lineHeight:1.2}}>{w.name}</div>
                            <div style={{fontFamily:"'DM Sans'",fontSize:10,color:"rgba(255,255,255,0.6)",letterSpacing:"0.04em"}}>{w.producer} · {w.vintage}</div>
                          </div>
                          <div style={{display:"flex",alignItems:"center",gap:4}}>
                            <span style={{fontSize:13}}>❤️</span>
                            <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:24,color:C.gold,fontWeight:300}}>{w.rating}</span>
                          </div>
                        </div>
                      </div>
                    )}
                    <div style={{padding:"14px 16px"}}>
                      {!w.photos?.[0]&&(
                        <>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:3}}>
                            <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:400,color:C.text,flex:1,paddingRight:8}}>{w.name}</div>
                            <div style={{display:"flex",alignItems:"center",gap:4}}>
                              <span style={{fontSize:11}}>❤️</span>
                              <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,color:isDeg?C.sepia:C.gold,fontWeight:300}}>{w.rating}</span>
                            </div>
                          </div>
                      ...
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        <div style={{display:"flex",gap:6,alignItems:"center"}}>
                          <div style={{width:2,height:12,background:isDeg?C.sepia:C.gold,borderRadius:1,opacity:0.6}} />
                          <span style={{fontFamily:"'DM Sans'",fontSize:9,color:isDeg?C.sepia:C.gold,background:isDeg?C.sepiaL:C.goldL,padding:"3px 9px",borderRadius:20,letterSpacing:"0.08em",textTransform:"uppercase"}}>{isDeg?"Degustado":w.style}</span>
                          {w.special&&<span style={{color:C.gold,fontSize:11}}>★</span>}
                        </div>
                        {!isDeg&&<Qty value={w.bottles} onChange={v=>update({...w,bottles:v})} accent={C.gold} />}
                      </div>
                    </div>
                  </div>
                );
        ...
        <div style={{padding:"0 24px"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:32}}>
            {[
              {label:"Tinto", cor:C.wine},
              {label:"Branco", cor:C.gold},
              {label:"Rose", cor:"#B05070"},
              {label:"Espumante", cor:"#4A8068"},
              {label:"Especiais", cor:C.gold},
              {label:"Todos", cor:C.muted},
            ].map(({label,cor})=>{
              const count = label==="Todos"?winesAtivos.length:contaTipo(label);
              return (
                <button key={label} onClick={()=>setFiltroAtivo(label)}
                  style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:8,padding:"18px 16px",display:"flex",flexDirection:"column",cursor:"pointer",textAlign:"left",gap:6}}>
            ...
            <button onClick={()=>setFiltroAtivo("Degustados")}
              style={{background:C.card,border:`1px solid ${C.sepia}40`,borderRadius:8,padding:"18px 16px",display:"flex",flexDirection:"column",cursor:"pointer",textAlign:"left",gap:6}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:32}}>
                {filtroCasta.map(c=>(
                  <button key={c} onClick={()=>setFiltroAtivo(c)}
                    style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:8,padding:"16px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer"}}>
                    <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,color:C.text,fontWeight:300,fontStyle:c==="Blend"?"italic":"normal"}}>{c}</span>
                    <span style={{fontFamily:"'DM Sans'",fontSize:10,color:C.muted,letterSpacing:"0.06em"}}>{contaCasta(c)} rótulo{contaCasta(c)!==1?"s":""}</span>
                  </button>
                ))}
              </div>
            </>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:24}}>
                {filtrosPais.map(p=>(
                  <button key={p} onClick={()=>setFiltroAtivo(p)}
                    style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:8,padding:"16px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer"}}>
                    <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,color:C.text,fontWeight:300}}>{p}</span>
                    <span style={{fontFamily:"'DM Sans'",fontSize:10,color:C.muted,letterSpacing:"0.06em"}}>{contaPais(p)} rótulo{contaPais(p)!==1?"s":""}</span>
                  </button>
                ))}
              </div>
            </>
...
export default function App() {
  ...
  if (loading) return (
    <div style={{maxWidth:430,margin:"0 auto",height:"100vh",background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:20}}>
      <style>{G}</style>
      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:11,color:C.gold,letterSpacing:"0.3em",textTransform:"uppercase",marginBottom:8}}>✦</div>
      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:30,fontWeight:300,letterSpacing:"0.2em",color:C.text}}>MEMORAVIN</div>
      <div style={{fontFamily:"'DM Sans'",fontSize:9,color:C.muted,letterSpacing:"0.2em",textTransform:"uppercase"}}>Adega & Memórias</div>
    </div>
  );
  ...
  return (
    <div style={{maxWidth:430,margin:"0 auto",height:"100vh",background:C.bg,display:"flex",flexDirection:"column",position:"relative",overflow:"hidden"}}>
      <style>{G}</style>
      <style>{`*{box-sizing:border-box;margin:0;padding:0}::-webkit-scrollbar{display:none}input,textarea,select{-webkit-appearance:none}::placeholder{color:#3A3A3E}body{background:#0D0D0F}`}</style>

      {/* Header MEMORAVIN */}
      <div style={{padding:"16px 24px 14px",borderBottom:`1px solid ${C.border}`,background:C.bg,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div style={{display:"flex",alignItems:"baseline",gap:10}}>
          <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,fontWeight:300,letterSpacing:"0.2em",color:C.text}}>MEMORAVIN</span>
        </div>
        <div style={{fontFamily:"'DM Sans'",fontSize:8,color:C.muted,letterSpacing:"0.2em",textTransform:"uppercase"}}>Adega & Memórias</div>
      </div>
      ...
      <div style={{position:"absolute",bottom:0,left:0,right:0,background:C.card,borderTop:`1px solid ${C.border}`,paddingTop:"10px",paddingBottom:"calc(12px + env(safe-area-inset-bottom))",display:"flex",justifyContent:"space-around",zIndex:40}}>
        {tabs.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{background:"none",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:5,flex:1}}>
            <span style={{fontSize:18,color:tab===t.id?C.gold:C.muted}}>{t.icon}</span>
            <span style={{fontFamily:"'DM Sans'",fontSize:8,letterSpacing:"0.14em",textTransform:"uppercase",color:tab===t.id?C.gold:C.muted,fontWeight:tab===t.id?500:300}}>{t.label}</span>
          </button>
        ))}
      </div>
    </div>
